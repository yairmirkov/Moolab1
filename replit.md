...
-   **UI/UX**:
    ...
    -   **Central Hub Navigation**: `Hub.tsx` is the main menu with animated floating cards for Lab 🧪, Tank 🦈, Vault 🏦. Replaces the old persistent bottom tab bar. `activeTab` state (`"hub" | "lab" | "tank" | "vault"`) controls which view is displayed. All navigation goes through `navigateTo()` helper which stops ElevenLabs audio when leaving Lab. Feed is hidden via `display:none` when not on Lab tab (state preserved). Tank and Vault have ← back buttons. Profile dashboard has "🏠 Back to Hub" button. Hub pre-fetches stock prices on mount.
    -   **FeedContext**: Global state context (`FeedContext.tsx`) preserving feed data across tab switches — `currentData`, `activeSlideIndex`, `completedSlides`, `slideAnswers`, `isFetchingMore`, `introBurned`. Wrapped via `FeedProvider` in `main.tsx`.
    -   **Burn-After-Reading Intro**: Intro card removed from feed after first scroll past it. `burnIntro()` splices intro from lessons array and resets scroll. `resetFeedSession()` resets on new session.
